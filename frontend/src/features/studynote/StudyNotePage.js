...
export default function StudyNotePage() {
  ...
  // Persisted (not plain useState): study-note generation runs for minutes
  // in the background, so a student who steps away mid-generation needs to
  // land back on this same wizard step on return — not step one — so
  // StudyNoteGenerator (and the job-polling it owns) remounts immediately.
  // See GeneratePage.js for the same fix on the worksheet studio.
  const [selectedClass, setSelectedClass] = usePersistedState("wizard:studynote:class", "");
  const [selectedSubject, setSelectedSubject] = usePersistedState("wizard:studynote:subject", "");
  const [selectedChapter, setSelectedChapter] = usePersistedState("wizard:studynote:chapter", "");
  const [selectedTopicId, setSelectedTopicId] = usePersistedState("wizard:studynote:topic", "");
