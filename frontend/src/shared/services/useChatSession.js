...
export function useChatSession({ studentId, subjectId, chapterId = null, topicId = null, language = "bangla" }) {
  ...
  // Per Step 3.9 / the job contract: POST /chat/quiz/generate now dispatches
  // to a worker (202 {job_id}) instead of returning the quiz inline. Poll
  // until SUCCESS, then treat the job's `result` exactly like the old
  // synchronous response body — remember() still needs session_id out of it.
  const quizSet = useCallback(async (difficulty = "medium", count = 5) => {
    ...
    // A non-student caller still gets a 200 with {questions: [], message}
    // (see the job contract) — nothing to poll in that case.
