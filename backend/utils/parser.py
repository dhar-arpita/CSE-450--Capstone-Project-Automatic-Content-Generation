...
def _replace_images(md: str, page_images, page_num: int) -> str:
    ...
    for img in page_images:
        ...
        if desc.upper() == "SKIP":
            ...
            # Use a function (not an f-string) as the replacement so re.sub
            # treats `desc` as a literal value. A string replacement gets
            # scanned for escape codes like \1 or \g<name>; if `desc`
            # contains LaTeX such as \perp or \pi (common in this content),
            # that scan fails with "bad escape \p ...".
            md = tag_pattern.sub(lambda m, d=desc: f'\n[Figure: {d}]\n', md)
    return md
