...
from core.config import gemini_client, generate_content_with_fallback
...
#     try:
#         # Try a direct parse first since response_mime_type handles most structures
#         descriptions = json.loads(raw)
#     except json.JSONDecodeError as e:
#         print(f"[Warning] Direct JSON parse failed ({e}). Applying bulletproof escape strategy...")
        
#         # ── ZERO-REGEX BULLETPROOF STRATEGY ──
#         # Step 1: Temporarily convert legitimate JSON structural escapes to placeholders
#         processed = raw.replace('\\"', '___QUOTE___').replace('\\\\', '___BACKSLASH___')
        
#         # Step 2: Safely double-escape EVERY remaining standalone backslash character
#         processed = processed.replace('\\', '\\\\')
        
#         # Step 3: Restore the structural placeholders safely back to valid JSON format
#         processed = processed.replace('___QUOTE___', '\\"').replace('___BACKSLASH___', '\\\\')
        
#         descriptions = json.loads(processed)
...
"""
    ...
    try:
        # Try direct load first (response_mime_type handles 99% of valid JSON formatting)
        descriptions = json.loads(raw)
    except json.JSONDecodeError as e:
        print(f"[Warning] Direct JSON parse failed ({e}). Applying bulletproof escape strategy...")
        ...
        # Step 2: Safely double-escape EVERY remaining standalone backslash character (\p, \l, \u, etc.)
        processed = processed.replace('\\', '\\\\')
...
"""
    ...
    # Enforce structured JSON constraint directly via the Gemini API configuration
    response = generate_content_with_fallback(
        model="gemini-2.5-flash",
        contents=prompt,
        config={"response_mime_type": "application/json"}
    ...
    # Safely handle potential internal backslashes without triggering regex engine escape crashes
    try:
        topic_names = json.loads(raw)
    except json.JSONDecodeError as e:
        print(f"[Warning] Topic JSON parse failed ({e}). Applying zero-regex escape strategy...")
        # Step 1: Temporarily hide valid structural escapes
        processed = raw.replace('\\"', '___QUOTE___').replace('\\\\', '___BACKSLASH___')
        # Step 2: Escaping all standalone slashes safely (\l, \u, \c, etc.)
        processed = processed.replace('\\', '\\\\')
        # Step 3: Restoring structural layout markers
        processed = processed.replace('___QUOTE___', '\\"').replace('___BACKSLASH___', '\\\\')
        
        topic_names = json.loads(processed)
    ...
    # Generate descriptions for all extracted topics
    descriptions = generate_topic_descriptions(topic_names, chapter_name, full_text)
    ...
    for name in topic_names:
        ...
        if existing:
            topic_id = existing.topic_id
            existing.description = description   # update description field
            print(f"  [Topic] Already exists: '{name}' (id={topic_id})")
        else:
            new_topic = Topic(
                chapter_id=chapter_id, 
                name=name,
                description=description    # save new description field
            )
            db.add(new_topic)
            db.flush()
            topic_id = new_topic.topic_id
