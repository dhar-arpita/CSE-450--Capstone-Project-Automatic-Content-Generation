# parser_v2 output for `physics_ch2.pdf`
...
[Figure: This figure illustrates two types of motion using a sequence of airplanes.
On the left side, labeled "Not translational motion," the airplanes are shown following a curved path, indicating that their orientation changes as they move, which represents rotational or circular motion.
On the right side, labeled "Translational motion," the airplanes maintain the same orientation while moving along a curved path, indicating that the entire object moves without any rotation.]
...
[Figure: This figure illustrates an example of simple harmonic motion (SHM). It depicts a person swinging back and forth on a swing, representing linear oscillatory motion where the restoring force is proportional to the displacement from the equilibrium position. The motion shown is periodic, repeating in the same manner over regular intervals.]
...
To differentiate vector quantities from scalar quantities they are written in bold font (e.g. **x**, **y** or **A**, **B**). In books or in computer printing it is easy to write anything in bold form. But when anyone writes on a paper then to express anything as vector, a small arrow is used over it ($\vec{x}, \vec{y}$ or $\vec{A}, \vec{B}$).
...
[Figure: This diagram illustrates the concept of vector quantities using arrows labeled A, B, C, and D. Each arrow represents a vector with both magnitude and direction, where the length and orientation of the arrows indicate different vector quantities and their respective directions. This visual helps to differentiate vectors from scalar quantities by showing that vectors have both a numerical value and a direction component.]
...
[Figure: This diagram illustrates the concepts of distance and displacement along a curved path. Point A is the starting position, and the path is marked with numbered segments (0 to 7) representing different points along the route. The actual path traveled is denoted by the curved line, while the straight-line distance from the starting point A to the ending point B represents the displacement. The diagram shows that distance is the total length of the path traveled (sum of all segments), whereas displacement is the straight-line distance between the initial and final positions.]
Figure 2.04 : Curved path trakelled by a cycle from point A.
...
Therefore, $$v = \frac{3 \text{ km}}{20 \text{ minutes}} = \frac{3 \times 1000 \text{ m}}{20 \times 60 \text{ s}} = 2.5 \text{ m/s}$$
...
[Figure: This image depicts a person spinning a ball on a string in a circular path. It visually represents circular motion, where the object moves at a constant speed along the curved trajectory, illustrating how direction continuously changes even if the speed remains uniform. This aligns with the concept that uniform speed does not necessarily mean uniform velocity when the direction changes.]
...
[Figure: This graph shows the relationship between velocity and time for an object. The graph consists of four labeled segments (A, B, C, and D) where the velocity changes over time, indicating periods of acceleration and deceleration. The slope of each segment represents the rate of change of velocity, which corresponds to acceleration. Flat segments indicate constant velocity with no acceleration.]
...
$$v = at$$
We have already said that all the discussions done so for are true for uniform acceleration. If the object does not move with uniform acceleration, then it is not so easy to find the acceleration from the initial and final velocities.
[Figure: This graph depicts the relationship between velocity and time for an object undergoing uniformly accelerated motion. The horizontal axis represents time, while the vertical axis represents velocity. The graph shows an initial velocity labeled as "u" and a final velocity labeled as "v" after a time interval "t," illustrating a linear increase in velocity over time.
The equation provided in the nearby text, acceleration = (final velocity - initial velocity) / time, can be transcribed as:
$$a = rac{(v - u)}{t}$$
...
Except time (t) the magnitude of these quantities may be positive or negative along a strait line We will not consider them as vectors and will talk about their magnitude only.
Discussions about almost al the relationship among these quantities, are done already, only one is left, which is the relationship for the distances. If an object has no acceleration, then there is no change in its velocity. Then the initial velocity and the final velocity will be equal i.e.(u = v). Therefore the distance covered will be,
$$s = vt$$
...
$$V = u + \frac{1}{2}at$$
[Figure: This graph depicts the relationship between velocity and time for an object undergoing uniformly accelerated motion. The graph shows velocity increasing linearly over time, with the average velocity (denoted as \( V \)) for the entire time interval from 0 to \( t \) being represented by the velocity at the midpoint of the interval, \( t/2 \). The equation \( V = rac{u + v}{2} \) is illustrated, where \( u \) is the initial velocity and \( v \) is the final velocity.]
...
Thus the acceleration of the car $-0.089 \text{ m/s}^2$ or deceleration $0.089 \text{ m/s}^2$
**Question:** The velocity of a bullet is $1.5 \text{ km/s}$. It has penetrated $10 \text{ cm}$ of a wall. What is the deceleration of the bullet?
...
$$
\begin{aligned} v &= u + gt \\ h &= ut + \frac{1}{2}gt^2 \\ v^2 &= u^2 + 2gh \end{aligned}
$$
...
$$\text{Therefore,} \quad h = \frac{u^2}{2g} = \frac{(41.67)^2}{2 \times 9.8} \text{ m} = 88.59 \text{ m}$$
...
$$\text{Therefore,} \quad h = \frac{(10,000)^2}{2 \times 9.8} \text{ m} = 5,102,000 \text{ m} = 5,102 \text{ km}$$
Though it seems that there is no mistake anywhere but actually the answer is not correct. This is because we have considered the value of acceleration due to gravity is 9.8 m/s², this is true for the distances near the surface of earth. But if we go far from the earth the value of $g$ will decrease. When we deduced the equation
$$v^2 = u^2 - 2gh$$
...
[Figure: This figure shows three related graphs depicting the motion of an object over time:
1. The top graph plots **distance (m)** against **time (s)**, showing a curve that indicates the object is accelerating as the distance increases non-linearly with time.
2. The middle graph plots **velocity (m/s)** against **time (s)**, showing a linear increase, which suggests the object has a constant acceleration.
3. The bottom graph plots **acceleration (m/s²)** against **time (s)**, showing a constant acceleration of approximately 2 m/s² over the entire time period.
The graphs collectively illustrate how the object's position, velocity, and acceleration change over time, with the velocity-time graph being derived from the distance-time graph and the acceleration-time graph derived from the velocity-time graph.]
...
### Investigation 2.01
**Determination of the average speed of an object rolling over a slope.**
...
[Figure: This image depicts an inclined plane experiment setup used to study the motion of an object rolling down a slope. The diagram shows a rectangular board inclined at an angle θ with height h, and length l, illustrating how the slope's steepness (angle θ) can be varied to observe the effect on the rolling object’s motion. The setup is likely used to investigate how different incline angles affect the speed and acceleration of the rolling object over the same distance.]
...
Let us assume that you can count up to forty-five in fifteen seconds. In that case, you can utter every number in approximately $15/45 = 1/3$ second. Let the marble or pencil or pen move on the inclined plane and start counting one, two, three and so on find up to which number you could count for the marble, pencil or pen to reach the lower end of the inclined plane. Calculate the actual time by multiplying with the correct multiple.
...
**Rotational motion:** Spreading your hands on opposite sides start rotating.
