...
| **5.5** | A | `docker-compose.prod.yml` | Add **Flower** behind Caddy basic-auth for queue visibility (depth, active tasks, failure counts). Needed *before* 5.3 — the ladder is much harder to interpret without queue introspection. | Flower shows the **two** queues (`generation`, `ingestion` — §4.1 dropped the strategy's original `rendering` queue) and live task counts. |
